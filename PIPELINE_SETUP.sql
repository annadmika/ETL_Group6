...
WHEN NOT MATCHED THEN INSERT (
    ...
    TOTAL_PRICE, ORIGIN_COUNTRY, FAIR_TRADE_CERTIFIED, ORGANIC_CERTIFIED, PURCHASE_TIME,
    SHIPPED_DATE, DELIVERED_DATE, REGION, NAME, STREET_ADDRESS, CITY, COUNTRY,
    POSTALCODE, PHONE, EMAIL, WAREHOUSE, SHIPPING_METHOD, DELIVERY_STATUS,
    PAYMENT_METHOD, PAYMENT_STATUS, DELIVERY_DELAY_DAYS, CARBON_SCORE, METADATA
) VALUES (
    src.TXID, src.RFID, src.CUSTOMER_ID, src.PRODUCT_ID, INITCAP(src.ITEM), UPPER(src.BAG_SIZE),
    src.UNIT_PRICE, src.QUANTITY, src.TOTAL_PRICE, src.ORIGIN_COUNTRY,
    src.FAIR_TRADE_CERTIFIED, src.ORGANIC_CERTIFIED,
    ...
    src.REGION, src.NAME, src.STREET_ADDRESS, src.CITY, src.COUNTRY,
    src.POSTALCODE, src.PHONE, src.EMAIL, src.WAREHOUSE, src.SHIPPING_METHOD,
    src.DELIVERY_STATUS, src.PAYMENT_METHOD, src.PAYMENT_STATUS, src.DELIVERY_DELAY_DAYS,
    src.CARBON_SCORE, src.METADATA
);
...
USING (
    SELECT *,
      CASE WHEN ESTIMATED_EMISSIONS_KGCO2E < 0 THEN NULL ELSE ESTIMATED_EMISSIONS_KGCO2E END AS ESTIMATED_EMISSIONS_CLEAN
    FROM RAW_CARBON_EMISSIONS_STREAM
    QUALIFY ROW_NUMBER() OVER (PARTITION BY RECORD_ID ORDER BY REPORTING_MONTH DESC) = 1
) AS src
...
-- ============================================
-- J. GOLD TABLES (separate)
-- ============================================

USE SCHEMA GOLD;

CREATE OR REPLACE TABLE GOLD_CLIENT_SUPPORT_ORDERS LIKE SILVER.CLIENT_SUPPORT_ORDERS_CLEAN;
CREATE OR REPLACE TABLE GOLD_CARBON_EMISSIONS LIKE SILVER.CARBON_EMISSIONS_CLEAN;

-- Gold Tasks: copy from Silver
CREATE OR REPLACE TASK task_gold_orders
  WAREHOUSE = PIPELINE_WH
  SCHEDULE = 'USING CRON 30 * * * * UTC'
  COMMENT = 'Refresh GOLD_CLIENT_SUPPORT_ORDERS from silver'
AS
MERGE INTO GOLD_CLIENT_SUPPORT_ORDERS AS tgt
USING SILVER.SILVER_ORDERS_STREAM AS src
ON tgt.TXID = src.TXID
WHEN MATCHED THEN UPDATE SET
    tgt.ITEM = src.ITEM,
    tgt.BAG_SIZE = src.BAG_SIZE,
...
WHEN NOT MATCHED THEN INSERT (
    TXID, ITEM, BAG_SIZE, PURCHASE_TIME, SHIPPED_DATE, DELIVERED_DATE, CARBON_SCORE
) VALUES (
    src.TXID, src.ITEM, src.BAG_SIZE, src.PURCHASE_TIME, src.SHIPPED_DATE, src.DELIVERED_DATE, src.CARBON_SCORE
);


CREATE OR REPLACE TASK task_gold_emissions
  WAREHOUSE = PIPELINE_WH
  SCHEDULE = 'USING CRON 35 * * * * UTC'
  COMMENT = 'Refresh GOLD_CARBON_EMISSIONS from silver'
AS
MERGE INTO GOLD_CARBON_EMISSIONS AS tgt
USING SILVER.SILVER_EMISSIONS_STREAM AS src
ON tgt.RECORD_ID = src.RECORD_ID
WHEN MATCHED THEN UPDATE SET
    tgt.ORDER_ID = src.ORDER_ID,
    tgt.WAREHOUSE_ID = src.WAREHOUSE_ID,
    tgt.WAREHOUSE_NAME = src.WAREHOUSE_NAME,
    tgt.WAREHOUSE_COUNTRY = src.WAREHOUSE_COUNTRY,
    tgt.ORIGIN_COUNTRY = src.ORIGIN_COUNTRY,
    tgt.DISTANCE_CLASS = src.DISTANCE_CLASS,
    tgt.SHIPPING_METHOD = src.SHIPPING_METHOD,
    tgt.SHIPMENTS_COUNT = src.SHIPMENTS_COUNT,
    tgt.AVG_BATCH_SIZE_KG = src.AVG_BATCH_SIZE_KG,
    tgt.ESTIMATED_EMISSIONS_KGCO2E = src.ESTIMATED_EMISSIONS_KGCO2E,
...
-- ============================================
-- K. COPY GOLD → STREAMLIT_APPS.GOLD_COPY
-- ============================================
...
CREATE OR REPLACE TASK task_copy_gold_orders_to_streamlit
  ...
  COMMENT = 'Copy GOLD_CLIENT_SUPPORT_ORDERS to STREAMLIT_APPS'
AS
MERGE INTO STREAMLIT_APPS.GOLD_COPY.CLIENT_SUPPORT_ORDERS AS tgt
...
CREATE OR REPLACE TASK task_copy_gold_emissions_to_streamlit
  ...
  COMMENT = 'Copy GOLD_CARBON_EMISSIONS to STREAMLIT_APPS'
AS
MERGE INTO STREAMLIT_APPS.GOLD_COPY.CARBON_EMISSIONS AS tgt
...
GRANT OPERATE ON TASK STREAMLIT_APPS.GOLD_COPY.task_copy_gold_orders_to_streamlit TO ROLE INGEST;
GRANT OPERATE ON TASK STREAMLIT_APPS.GOLD_COPY.task_copy_gold_emissions_to_streamlit TO ROLE INGEST;
...
ALTER TASK IF EXISTS STREAMLIT_APPS.GOLD_COPY.task_copy_gold_orders_to_streamlit RESUME;
ALTER TASK IF EXISTS STREAMLIT_APPS.GOLD_COPY.task_copy_gold_emissions_to_streamlit RESUME;

-- Run a single task immediately
EXECUTE TASK ECO_COFFEE_DWH.SILVER.task_clean_orders;
EXECUTE TASK ECO_COFFEE_DWH.SILVER.task_clean_emissions;
EXECUTE TASK ECO_COFFEE_DWH.GOLD.task_gold_orders;
EXECUTE TASK ECO_COFFEE_DWH.GOLD.task_gold_emissions;
EXECUTE TASK STREAMLIT_APPS.GOLD_COPY.task_copy_gold_orders_to_streamlit;
EXECUTE TASK STREAMLIT_APPS.GOLD_COPY.task_copy_gold_emissions_to_streamlit;
